    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="flex h-full min-h-[150px] w-full flex-col items-center justify-center rounded-xl border border-rose-500/20 bg-rose-500/5 p-6 text-center">
          ...
          <h4 className="mt-3 font-semibold text-slate-200">Panel Execution Failed</h4>
          <p className="mt-1 max-w-xs text-xs text-slate-500">
            {this.state.error?.message ?? 'An unexpected render error occurred inside this card.'}
          </p>
          <button
            onClick={this.handleReset}
            className="interactive-element mt-4 flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-800 px-3 py-1.5 text-xs font-medium text-slate-300 hover:bg-slate-700 hover:text-slate-200"
          >
            <RefreshCw size={12} />
            Reset State
          </button>
