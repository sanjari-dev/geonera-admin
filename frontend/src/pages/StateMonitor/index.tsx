...
function CustomTooltip({ active, payload, label }: any) {
  if (active && payload && payload.length) {
    return (
      <div className="rounded-xl border border-slate-700/60 bg-[#0F121D]/95 px-4 py-3 shadow-2xl backdrop-blur-md">
        <p className="font-mono text-xs font-semibold text-slate-300">{label}</p>
        ...
        {recent.data && recent.data.data.length > 0 && (
          <div className="h-full w-full overflow-auto">
            <table className="w-full border-collapse">
              <thead className="sticky top-0 z-10 bg-slate-950/90 backdrop-blur-md border-b border-slate-800/80">
                <tr>
              ...
              <tbody className="divide-y divide-slate-800/40">
                {recent.data.data.map((s, idx) => (
                  <tr key={s.id} className="interactive-element group border-b border-slate-800/30 hover:bg-slate-800/30 transition-colors duration-150">
                    <td className="px-4 py-2.5 font-mono text-xs font-bold uppercase text-slate-300">
                      {s.instrument?.name ?? s.instrumentId.slice(0, 8)}
                    </td>
                    <td className="px-4 py-2.5">
                      <span className={`rounded-md px-2 py-0.5 text-[10px] font-semibold tracking-wider ${s.jobType === 'TICK' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'bg-pink-500/10 text-pink-400 border border-pink-500/20'}`}>
                        {s.jobType}
                      </span>
                    </td>
